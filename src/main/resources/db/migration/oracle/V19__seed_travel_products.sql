...
SELECT
    d.DEST_ID,
    d.DEST_NAME || ' ' || CASE MOD(d.RN - 1, 10)
    ...
    '이 상품은 ' || d.DEST_NAME || ' 여행에서 자주 생기는 예약과 이동의 불편을 줄이도록 구성했습니다. ' ||
    ...
    '여권 또는 신분증을 준비해 주세요. 현지 기상·교통 상황에 따라 순서가 변경될 수 있습니다. ' ||
    ...
    'Q. 예약 즉시 확정되나요?' || CHR(10) || 'A. 대부분 5분 이내 확정되며 별도 확인 상품은 24시간 안에 안내됩니다.' || CHR(10) ||
    'Q. 날짜를 변경할 수 있나요?' || CHR(10) || 'A. 재고가 있는 경우 이용 48시간 전까지 1회 가능합니다.' || CHR(10) ||
    'Q. 어린이도 예약해야 하나요?' || CHR(10) || 'A. 옵션별 연령 기준을 확인해 주세요.',
    CASE MOD(d.RN - 1, 5)
        WHEN 0 THEN '09:00' WHEN 1 THEN '10:30' WHEN 2 THEN '16:30'
        WHEN 3 THEN '18:00' ELSE '항공편 도착 시간 또는 예약 시간' END,
    CASE MOD(d.RN - 1, 5)
        WHEN 0 THEN d.DEST_NAME || ' 메인 입구 안내 데스크'
        WHEN 1 THEN d.DEST_NAME || ' 인근 중앙역 2번 출구'
        WHEN 2 THEN d.DEST_NAME || ' 방문자 센터 앞'
        WHEN 3 THEN d.DEST_NAME || ' 구시가지 광장 시계탑'
        ELSE '예약 확정 후 바우처에 개별 안내' END,
    'https://demo.routemate.local/products/' || d.DEST_ID || '-' || d.RN,
    CASE MOD(d.RN - 1, 10)
...
WHERE d.RN <= 100
  AND NOT EXISTS (
      ...
      WHERE p.BOOKING_URL = 'https://demo.routemate.local/products/' || d.DEST_ID || '-' || d.RN
  );
...
CROSS JOIN (
    SELECT 1 AS OPTION_NO FROM DUAL
