...
public class AdminRecommendationService {
    ...
    public AdminRecommendationResponse.Item update(Long recommendId, AdminRecommendationRequest request) {
        ...
        var destination = getDestination(request.destinationId());
        validatePeriod(request);
        recommendation.update(destination, request.displayStartDt(), request.displayEndDt(), normalizeSortOrder(request.sortOrder()), normalizeUseYn(request.useYn()));
        return AdminRecommendationResponse.Item.from(recommendation);
    }

    private com.trip.routemate.destination.domain.Destination getDestination(Long destinationId) { return destinationRepository.findById(destinationId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "여행지를 찾을 수 없습니다.")); }
