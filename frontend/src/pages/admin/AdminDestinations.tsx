...
export const AdminDestinations = () => {
  ...
  const saveCountry = async (event: React.FormEvent) => {
    event.preventDefault(); if (uploadingImage) return; setSaving(true); setError(null);
    try {
  ...
  const saveRegion = async (event: React.FormEvent) => {
    event.preventDefault(); if (uploadingImage) return; if (!selectedCountryId) return;
  ...
  return <>
    ...
    <section className="mt-6 rounded-[24px] border border-white/10 bg-white/[0.03] p-5 sm:p-6">
      ...
      <form onSubmit={(event) => void savePlace(event)} className="mt-5 grid gap-3 lg:grid-cols-4">
        ...
        <div className="lg:col-span-2"><ImageUpload disabled={saving} value={placeForm.imageUrl} endpoint="/api/admin/images/DESTINATION" onUploadingChange={setUploadingImage} onChange={(imageUrl) => setPlaceForm((current) => ({ ...current, imageUrl }))} /></div>
        ...
        <button type="submit" disabled={saving || uploadingImage || !placeForm.countryId || !placeForm.regionId || !placeForm.category} className="h-11 rounded-xl bg-indigo-500 px-4 text-xs font-bold text-white disabled:opacity-50 lg:col-span-4">{saving ? '저장 중...' : editingPlaceId ? '플레이스 수정' : '플레이스 등록'}</button>
      </form>
