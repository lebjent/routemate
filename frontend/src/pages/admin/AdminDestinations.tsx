...
type Place = { destinationId: number; destName: string; destDesc: string | null; countryId: number; countryName: string; regionId: number; regionName: string; category: string; imageUrl: string | null; mapLat: number; mapLng: number; likeCount: number };
...
const emptyPlace = { destName: '', destDesc: '', countryId: '', regionId: '', category: 'SIGHTSEEING', imageUrl: '', mapLat: '', mapLng: '' };
...
export const AdminDestinations = () => {
  ...
  const savePlace = async (event: React.FormEvent) => {
    ...
    } catch (saveError) { setError(axios.isAxiosError(saveError) ? saveError.response?.data?.detail || '여행지를 저장하지 못했습니다.' : '여행지를 저장하지 못했습니다.'); } finally { setSaving(false); }
  };
  ...
  return <>
    ...
    <section className="mt-6 rounded-[24px] border border-white/10 bg-white/[0.03] p-5 sm:p-6">
      <div className="flex items-center justify-between"><div><p className="text-xs font-bold tracking-[0.16em] text-indigo-300">TRAVEL PLACE</p><h2 className="mt-1 text-lg font-bold text-white">여행지 관리</h2><p className="mt-1 text-xs text-slate-500">등록된 국가와 지역에 여행지와 대표 이미지를 연결합니다.</p></div>{editingPlaceId ? <button type="button" onClick={() => { setEditingPlaceId(null); setPlaceForm({ ...emptyPlace, countryId: String(selectedCountryId ?? '') }); }} className="text-xs text-slate-500">수정 취소</button> : null}</div>
      <form onSubmit={(event) => void savePlace(event)} className="mt-5 grid gap-3 lg:grid-cols-4">
        <select required value={placeForm.countryId} onChange={(event) => { const value = event.target.value; setSelectedCountryId(Number(value) || null); setPlaceForm((current) => ({ ...current, countryId: value, regionId: '' })); }} className="h-11 rounded-xl border border-white/10 bg-slate-900 px-3 text-sm text-slate-300"><option value="">국가 선택</option>{data?.countries.map((country) => <option key={country.countryId} value={country.countryId}>{country.countryName}</option>)}</select>
        <select required value={placeForm.regionId} onChange={(event) => setPlaceForm((current) => ({ ...current, regionId: event.target.value }))} className="h-11 rounded-xl border border-white/10 bg-slate-900 px-3 text-sm text-slate-300"><option value="">지역 선택</option>{regions.map((region) => <option key={region.regionId} value={region.regionId}>{region.regionName}</option>)}</select>
        <input required value={placeForm.destName} onChange={(event) => setPlaceForm((current) => ({ ...current, destName: event.target.value }))} placeholder="여행지명" className="h-11 rounded-xl border border-white/10 bg-slate-950/60 px-3 text-sm text-white outline-none" />
        <select required value={placeForm.category} onChange={(event) => setPlaceForm((current) => ({ ...current, category: event.target.value }))} className="h-11 rounded-xl border border-white/10 bg-slate-900 px-3 text-sm text-slate-300"><option value="SIGHTSEEING">관광</option><option value="FOOD">맛집</option><option value="CULTURE">문화</option><option value="SHOPPING">쇼핑</option><option value="NATURE">자연</option></select>
        ...
        <textarea value={placeForm.destDesc} onChange={(event) => setPlaceForm((current) => ({ ...current, destDesc: event.target.value }))} placeholder="여행지 설명" className="h-24 rounded-xl border border-white/10 bg-slate-950/60 px-3 py-3 text-sm text-white outline-none lg:col-span-2" />
        ...
        <button type="submit" disabled={saving || !selectedCountryId} className="h-11 rounded-xl bg-indigo-500 px-4 text-xs font-bold text-white disabled:opacity-50 lg:col-span-4">{saving ? '저장 중...' : editingPlaceId ? '여행지 수정' : '여행지 등록'}</button>
      </form>
      <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead className="border-b border-white/10 text-[11px] text-slate-600"><tr><th className="px-3 py-3">대표 이미지</th><th className="px-3 py-3">여행지</th><th className="px-3 py-3">국가·지역</th><th className="px-3 py-3">카테고리</th><th className="px-3 py-3 text-right">관리</th></tr></thead><tbody className="divide-y divide-white/5">{places.map((place) => <tr key={place.destinationId}><td className="px-3 py-3">{place.imageUrl ? <img src={place.imageUrl} alt="" className="h-12 w-20 rounded-lg object-cover" /> : <div className="flex h-12 w-20 items-center justify-center rounded-lg bg-white/5 text-[10px] text-slate-600">이미지 없음</div>}</td><td className="px-3 py-3 text-sm font-semibold text-slate-200">{place.destName}</td><td className="px-3 py-3 text-xs text-slate-500">{place.countryName} · {place.regionName}</td><td className="px-3 py-3 text-xs text-slate-500">{place.category}</td><td className="px-3 py-3 text-right"><button type="button" onClick={() => editPlace(place)} className="text-xs font-semibold text-indigo-300">수정</button></td></tr>)}{places.length === 0 ? <tr><td colSpan={5} className="px-3 py-10 text-center text-xs text-slate-600">등록된 여행지가 없습니다.</td></tr> : null}</tbody></table></div>
    </section>
  </>;
};
