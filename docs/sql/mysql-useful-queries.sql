/*
 * RouteMate 운영 조회 쿼리 모음 (MySQL 8.0+)
 * 기준 스키마: Flyway V1 ~ V19
 * 모든 쿼리는 SELECT 전용이며 서로 독립적으로 실행할 수 있습니다.
 * 검색 조건은 SET @변수명 = 값; 형태로 지정합니다. 사용하지 않는 조건은 NULL입니다.
 *
...
--------------------------------------------------------------------------------
-- 04. 상품 검색 및 필터
-- 예시: SET @keyword = '야경'; SET @country_code = NULL;
--------------------------------------------------------------------------------
SET @keyword = NULL;
...
WHERE (@keyword IS NULL
       OR UPPER(p.PRODUCT_NAME) LIKE CONCAT('%', UPPER(@keyword), '%')
       OR UPPER(d.DEST_NAME) LIKE CONCAT('%', UPPER(@keyword), '%')
       OR UPPER(p.PROVIDER_NAME) LIKE CONCAT('%', UPPER(@keyword), '%'))
...
LEFT JOIN TB_DESTINATION d ON d.REGION_ID = r.REGION_ID
LEFT JOIN TB_TRAVEL_PRODUCT p ON p.DEST_ID = d.DEST_ID
GROUP BY c.COUNTRY_NAME, r.REGION_CODE, r.REGION_NAME, r.SORT_ORDER
...
SELECT p.PRODUCT_TYPE,
       ...
       ROUND(AVG(COALESCE(option_summary.OPTION_COUNT, 0)), 1) AS AVG_OPTION_COUNT
FROM TB_TRAVEL_PRODUCT p
LEFT JOIN (
...
SELECT p.PRODUCT_ID,
       ...
       CONCAT(
           CASE WHEN p.PRODUCT_SUMMARY IS NULL THEN '요약, ' ELSE '' END,
           CASE WHEN p.PRODUCT_DESC IS NULL THEN '설명, ' ELSE '' END,
           CASE WHEN p.IMAGE_URL IS NULL THEN '대표이미지, ' ELSE '' END,
           CASE WHEN p.DETAIL_IMAGE_URL IS NULL THEN '상세이미지, ' ELSE '' END,
           CASE WHEN p.COURSE_TEXT IS NULL THEN '코스, ' ELSE '' END,
           CASE WHEN p.INCLUDED_TEXT IS NULL THEN '포함사항, ' ELSE '' END,
           CASE WHEN p.EXCLUDED_TEXT IS NULL THEN '불포함사항, ' ELSE '' END,
           CASE WHEN p.USAGE_GUIDE_TEXT IS NULL THEN '이용안내, ' ELSE '' END,
           CASE WHEN p.CANCELLATION_POLICY_TEXT IS NULL THEN '취소규정, ' ELSE '' END,
           CASE WHEN p.FAQ_TEXT IS NULL THEN 'FAQ, ' ELSE '' END
       ) AS MISSING_FIELDS
FROM TB_TRAVEL_PRODUCT p
JOIN TB_DESTINATION d ON d.DEST_ID = p.DEST_ID
...
SELECT p.DEST_ID,
       ...
       GROUP_CONCAT(p.PRODUCT_ID ORDER BY p.PRODUCT_ID SEPARATOR ', ') AS PRODUCT_IDS
FROM TB_TRAVEL_PRODUCT p
JOIN TB_DESTINATION d ON d.DEST_ID = p.DEST_ID
...
SELECT rec.RECOMMEND_ID,
       ...
       d.DEST_NAME,
       d.CATEGORY,
       COALESCE(rec.IMAGE_URL, d.IMAGE_URL) AS DISPLAY_IMAGE_URL,
...
JOIN TB_COUNTRY c ON c.COUNTRY_ID = d.COUNTRY_ID
WHERE rec.USE_YN = 'Y'
  AND CURRENT_TIMESTAMP(6) BETWEEN rec.DISPLAY_START_DT AND rec.DISPLAY_END_DT
...
--------------------------------------------------------------------------------
SELECT DATE_FORMAT(JOIN_DT, '%Y-%m') AS JOIN_MONTH,
       COUNT(*) AS JOIN_COUNT,
...
FROM TB_USER_MSTR
WHERE JOIN_DT >= DATE_SUB(DATE_FORMAT(CURRENT_DATE, '%Y-%m-01'), INTERVAL 11 MONTH)
  AND DEL_YN = 'N'
GROUP BY DATE_FORMAT(JOIN_DT, '%Y-%m')
ORDER BY JOIN_MONTH;
